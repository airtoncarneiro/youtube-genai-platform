...
## Objetivo e contexto de aprendizado

Este projeto também funciona como um laboratório prático de evolução profissional
de Engenharia de Dados para AI Engineering e GenAI Engineering. A implementação
usa um problema concreto — acompanhar dados públicos do YouTube — para exercitar
Lakehouse Architecture, Delta Lake, Databricks, Python, APIs, SQL, observabilidade
e práticas de MLOps/LLMOps.

O objetivo de longo prazo é transformar dados operacionais em análises semânticas
e insights assistidos por IA. O README distingue deliberadamente a fundação já
executável das capacidades de GenAI que permanecem planejadas.

## Caso de uso e evolução da arquitetura

Hoje a plataforma acompanha vídeos e canais públicos, incluindo metadados,
métricas, comentários e respostas. A partir dessa base, a evolução prevista é
responder perguntas como:

- quais vídeos apresentaram maior crescimento ou engajamento;
- quais temas e reclamações aparecem com maior frequência;
- como a percepção da audiência varia entre vídeos e canais;
- quais tendências podem ser identificadas ao longo do tempo.

Essas perguntas representam o objetivo analítico futuro; os dashboards atuais
já entregam métricas operacionais, evolução temporal e rastreabilidade, mas não
constituem ainda uma camada de NLP, RAG ou agentes.

```text
YouTube Data API
        │
        ▼
raw + control              entrada e orquestração
        │
        ▼
silver + snapshots         entidades atuais e histórico
        │
        ▼
gold                       modelos analíticos futuros
        │
        ▼
NLP / embeddings / Vector Search
        │
        ▼
RAG / AI Agents             capacidades planejadas
```

## Stack de tecnologias

| Domínio | Tecnologias atuais ou previstas |
|---|---|
| Engenharia de dados | Databricks, Apache Spark, Delta Lake, Unity Catalog, Databricks Workflows, Python, SQL e Git |
| Integração | YouTube Data API, cliente Python, retries e controle de quota |
| Observabilidade | Logs estruturados, tabelas de execução, dashboards e alertas SQL |
| GenAI planejada | NLP, embeddings, Vector Search, RAG e agentes especializados |
| Qualidade e operação | Testes Python, Ruff, Poetry, Declarative Automation Bundles e snapshots imutáveis |

## Índice

- [O que o projeto faz hoje](#o-que-o-projeto-faz-hoje)
- [Objetivo e contexto de aprendizado](#objetivo-e-contexto-de-aprendizado)
- [Caso de uso e evolução da arquitetura](#caso-de-uso-e-evolução-da-arquitetura)
...
- [Roadmap de aprendizado e evolução](#roadmap-de-aprendizado-e-evolução)
...
### Fase 1 — Fundação de Engenharia de Dados — implementada

- ingestão de vídeos, canais, comentários e respostas;
- processamento incremental e idempotente;
- arquitetura Lakehouse com Delta Lake e Unity Catalog;
- Jobs serverless, descoberta automática por canal e snapshots temporais;
- observabilidade operacional, alertas e dashboards.

### Fase 2 — Qualidade e dados não estruturados — parcialmente planejada

- marcar comentários e replies ausentes em varreduras completas sem apagar
  histórico;
- adicionar métricas de qualidade e alertas de erro ou duração anômala;
- ampliar limpeza e normalização de textos públicos.

Transcrições de canais de terceiros permanecem fora do escopo por questões de
direitos autorais e termos de uso do YouTube. Para conteúdo do próprio
proprietário, uma futura implementação deverá usar a API oficial, OAuth e as
permissões apropriadas.

### Fase 3 — Analytics e GenAI — planejada

- criar modelos `gold` de crescimento, engajamento e comparação;
- aplicar NLP, classificação de temas e análise de sentimentos;
- gerar embeddings e disponibilizar busca semântica com Vector Search.

### Fase 4 — RAG — planejada

- construir um pipeline de recuperação contextual;
- responder perguntas sobre vídeos, canais e comentários;
- avaliar a qualidade, a fundamentação e a segurança das respostas.

### Fase 5 — AI Agents — planejada
...
O roadmap é uma trilha de evolução, não uma declaração de que essas fases já
estão implantadas no workspace.
